...
def process_images(image_path, output_folder, brightness_range=(0.8, 1.2), contrast_range=(0.9, 1.2), 
 ...
 # Save the augmented image
    ...
    pygame.mixer.init()
    pygame.mixer.music.load("D:/skrui/pkkr.wav")  # Specify the absolute path to the .wav file
...
def start_surveillance():
    ...
    # Capture frames from each camera
        ...
        if ret0 and ret1 and ret2 and ret3:
            # Define the color and thickness for borders
            border_color = (255, 255, 255)  # White border
            border_thickness = 10  # Thickness of the margin

            # Add white margins around each frame
            frame0 = cv2.copyMakeBorder(frame0, border_thickness, border_thickness, border_thickness, border_thickness, cv2.BORDER_CONSTANT, value=border_color)
            frame1 = cv2.copyMakeBorder(frame1, border_thickness, border_thickness, border_thickness, border_thickness, cv2.BORDER_CONSTANT, value=border_color)
            frame2 = cv2.copyMakeBorder(frame2, border_thickness, border_thickness, border_thickness, border_thickness, cv2.BORDER_CONSTANT, value=border_color)
            frame3 = cv2.copyMakeBorder(frame3, border_thickness, border_thickness, border_thickness, border_thickness, cv2.BORDER_CONSTANT, value=border_color)

            # Define spacing between frames
            spacing = 20  # 20 pixels between each frame
            spacer = np.full((frame0.shape[0], spacing, 3), border_color, dtype=np.uint8)  # Vertical spacer
            h_spacer = np.full((spacing, frame0.shape[1] * 2 + spacing, 3), border_color, dtype=np.uint8)  # Horizontal spacer

            # Stack frames in a 2x2 grid with spacing
            top_row = cv2.hconcat([frame0, spacer, frame1])
            bottom_row = cv2.hconcat([frame2, spacer, frame3])
            combined_frame = cv2.vconcat([top_row, h_spacer, bottom_row])

...
def process_image():
    ...
    for uploaded_image_path in uploaded_image_paths:
        ...
        process_images(totaln,uploaded_image_path, image_name, brightness_range, contrast_range, saturation_range, max_perspective_warp, 50)
        totaln += 50
    pygame.mixer.init()
    pygame.mixer.music.load("pkkr.wav")  # Specify the absolute path to the .wav file
    pygame.mixer.music.play()
  # 2500 Hz frequency, 1000 ms duration
...
def validate_login(login_screen, user_entry, password_entry):
    ...
    # Check if credentials match the allowed admin username and password
...
def start_surveillance():
    global left_frame, right_frame,folder_info_text  # Declare both left_frame and right_frame as global
    ...
    # Configure grid columns for the main frame
    main_frame.grid_columnconfigure(0, weight=2)  # Left frame (40%)
    main_frame.grid_columnconfigure(1, weight=8)  # Right frame (60%)

    # Create a left frame for the uploaded image and buttons with a border
    left_frame = tk.Frame(main_frame, bg='#1e1e1e', bd=2, relief=tk.SUNKEN)
    left_frame.grid(row=0, column=0, sticky="nsew", padx=(10, 5), pady=10)  # Use grid instead of pack

    # Create a right frame for the generated images and let it expand
    right_frame = tk.Frame(main_frame, bg='#1e1e1e')
    right_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 10), pady=10)  # Use grid instead of pack

    main_frame.grid_rowconfigure(0, weight=1)
    # Add a "Back to Home" button in the left frame
    back_button = tk.Button(left_frame, text="Back to Home", command=back_to_home, font=("Arial", 12), bg="#e74c3c", fg="white")
    back_button.pack(pady=10, padx=20)

    # Label to display folder info
    folder_info_text = tk.Label(left_frame, text="No folder selected\nNumber of images: 0", 
                                font=("Arial", 12), bg="#1e1e1e", fg="white", anchor="w", justify="left")
    folder_info_text.pack(pady=10, padx=5, anchor="w")
...
def upload_dataset():
    """Function to upload a dataset of images and update UI."""
    global train_button,folder_name
    ...
    if folder_path:
        ...
        if not image_files:
            folder_info_text.config(text=f"No valid images found in {folder_name}")
            return

        num_images = len(image_files)

        # Update the folder_info_label with folder name and number of images
        folder_info_text.config(text=f"Folder Name: {folder_name}\nNumber of images: {num_images}")

        display_generated_images(folder_path)
        # Create and show the Train button if it's not already created
        
        if train_button is None:
            train_button = tk.Button(left_frame, text="Train Model", command=lambda: train_model(folder_path),
                                     font=("Arial", 12), bg="#2ecc71", fg="white")
            train_button.pack(pady=(10, 5))
            
    else:
        folder_info_text.config(text="No folder selected\nNumber of images: 0")

def get_images_and_labels(path):
    """Load face images and corresponding labels from the given directory path."""
    try:
        image_paths = [os.path.join(path, f) for f in os.listdir(path) if f.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]
        face_samples = []
        ids = []
        ...
        for image_path in image_paths:
            ...
            # Extract user ID from file name (assuming format "user.id.jpg")
            file_name = os.path.basename(image_path)
            id = int(file_name.split('.')[0])  # Adjust as per naming convention
...
def train_model(folder_path):
    print("0000000000000000000000000000000000000000",folder_name)
    ...
    try:
        ...
        # Get training data
        faces, ids = get_images_and_labels(folder_path)
        ...
        # Save the model
        recognizer.write( f"{folder_name}.yml")
        print(f"Model trained with {len(np.unique(ids))} unique faces")
    except Exception as e:
...
def load_home_content(logo_image):
    ...
    button1 = tk.Button(button_frame, text=" FACE SKETCH ", font="Arial 15 bold", padx=10, bg="#f7a014",
    ...
    button4 = tk.Button(button_frame, text=" TRAIN  ", command=trainningsa, font="Arial 15 bold", padx=20, bg="#f7a014",
                        fg="white", pady=5, bd=10, highlightthickness=0, activebackground="#091428",
                        activeforeground="#1e1e1e")
    button4.pack(side=tk.LEFT, padx=(38, 20), anchor='e')  # Position button on the far right with padding
...
def trainningsa():
    global left_frame, surveillance_button  # Declare surveillance_button as global

    # Clear all widgets from the home screen to prepare for the Generate Dataset screen
    for widget in home_screen.winfo_children():
        widget.destroy()

    # Create a main frame to hold the Listbox and Surveillance button
    main_frame = tk.Frame(home_screen, bg='#1e1e1e')
    main_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

    # Add a "Back to Home" button above the Listbox
    back_button = tk.Button(main_frame, text="Back to Home", command=back_to_home, font=("Arial", 14), bg="#e74c3c", fg="white", relief="raised", bd=3, padx=15, pady=10)
    back_button.pack(pady=(0, 20))  # Padding to space it above the Listbox

    # Create a Listbox to display the .yml files with increased size
    yml_files_listbox = tk.Listbox(main_frame, selectmode=tk.SINGLE, bg="#2c3e50", fg="white", font=("Arial", 14), height=15, width=30, bd=2, relief="sunken")
    # Get the current working directory (same folder as the script)
    directory = os.getcwd()

    # List all files with .yml extension in the current directory
    yml_files = [f for f in os.listdir(directory) if f.endswith('.yml')]
    for file in yml_files:
        yml_files_listbox.insert(tk.END, file)
    yml_files_listbox.pack(pady=20)

    # Create a green "Surveillance" button (initially disabled)
    surveillance_button = tk.Button(main_frame, text="Surveillance", bg="green", fg="white", state=tk.DISABLED, font=("Arial", 16), width=20, height=2, relief="raised", bd=3)
    surveillance_button.pack(pady=20)

    # Function to handle the selection event
    def on_select(event):
        selected_file = yml_files_listbox.get(yml_files_listbox.curselection())
        print(f"Selected file: {selected_file}")

        # Enable the "Surveillance" button when a file is selected
        surveillance_button.config(state=tk.NORMAL)

    # Bind the selection event
    yml_files_listbox.bind('<<ListboxSelect>>', on_select)
...
def show_generate_dataset_screen():
    ...
    # Make sure the rows expand as well
    main_frame.grid_rowconfigure(0, weight=1)
...
uploaded_image_paths = []  # List to store paths of uploaded images
current_index = 0  # Start with the first image
image_label = None  # Initialize image label
run_button = None  # Initialize the run button
next_button = None  # Initialize the next button
previous_button = None  # Initialize the previous button

def upload_image():
    global uploaded_image_paths  # Access global uploaded image paths
    # Open file dialog to allow the user to select one or more image files
    file_paths = filedialog.askopenfilenames(filetypes=[("Image Files", "*.png;*.jpg;*.jpeg")])
    
    if file_paths:  # If at least one file was selected
        uploaded_image_paths = list(file_paths)  # Store the uploaded image paths as a list
        print(f"Images uploaded: {', '.join(uploaded_image_paths)}")  # Print file paths for confirmation

        # Display all uploaded images on the Generate Dataset screen
        display_image(uploaded_image_paths[current_index])  # Display the first image in the list

        # If this is the first image upload, display a "Run" button below the images
        global run_button
        if run_button is None:  # Ensure run button is only added once
            ...
            run_button.pack(pady=10)  # Position the "Run" button with padding below the uploaded images
...
def display_image(file_path):
    ...
    if next_button is None or previous_button is None:
        next_button = tk.Button(left_frame, text="Next", command=next_image)
        next_button.pack(side="right", padx=10)

        previous_button = tk.Button(left_frame, text="Previous", command=previous_image)
